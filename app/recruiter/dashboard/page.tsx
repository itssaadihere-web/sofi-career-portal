...
import { Building2, PlusCircle, Eye, Users, Briefcase, Loader2, X, ExternalLink, Sparkles, Edit3, UserCheck, ShieldCheck } from 'lucide-react'
...
export default function RecruiterDashboard() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const isAdminRole = recProfile?.role === 'admin' || recProfile?.is_admin || recProfile?.email?.toLowerCase() === 'saadi@joinsophi.com'

      // Fetch jobs: Admin Recruiter sees ALL jobs across portal; Regular recruiter sees only their own
      ...
      if (!isAdminRole) {
        jobQuery = jobQuery.eq('recruiter_id', session.user.id)
      }

      const { data: jobList } = await jobQuery
      ...
      if (jobList) {
        ...
        if (jobIds.length > 0) {
          ...
          if (!isAdminRole) {
            appQuery = appQuery.in('job_id', jobIds)
          }
  ...
  const isAdmin = recruiter?.role === 'admin' || recruiter?.is_admin || recruiter?.email?.toLowerCase() === 'saadi@joinsophi.com'
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 flex-wrap mt-0.5">
              ...
              {isAdmin && (
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-900 border border-purple-300 font-extrabold text-[11px] shadow-2xs">
                  <ShieldCheck className="h-3.5 w-3.5 text-purple-600" /> Admin Recruiter Mode (All Jobs View)
                </span>
              )}
            </div>
          </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase font-extrabold">
                <tr>
                  ...
                  {isAdmin && <th className="px-6 py-4">Posted By (Recruiter ID)</th>}
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          <div className="font-extrabold text-slate-900 text-xs">
                            {j.recruiter_profiles?.full_name || j.recruiter_profiles?.company_name || 'Employer'}
                          </div>
                          {j.recruiter_profiles?.email && (
                            <div className="text-[11px] text-slate-500 font-medium">{j.recruiter_profiles.email}</div>
                          )}
                          <div className="inline-flex items-center gap-1 bg-slate-100 border border-slate-200 rounded px-2 py-0.5 font-mono text-[10px] text-slate-700 font-semibold" title="Poster Recruiter ID">
                            <span>ID:</span>
                            <span className="truncate max-w-[130px]">{j.recruiter_id || 'N/A'}</span>
                          </div>
                        </div>
                      </td>
                    )}
